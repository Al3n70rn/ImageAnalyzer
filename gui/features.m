...
% Texture parameter calculations here (for ALL ROIs)!
% Check which attributes are selected

for i = 1:size(handles.study.imageSeriesList{handles.study.imageSeriesId}.roiList, 2)

%    ROIs = handles.study.imageSeriesList{handles.study.imageSeriesId}.roiList;
    currentROI = getRoiValues(handles.study, i);
   

    % STANDARD DEVIATION
    if get(handles.checkbox_std,'Value') == 1
        std_value = std(double(currentROI(:)))
    end
    
    % MEAN
    if get(handles.checkbox_mean,'Value') == 1
        mean_value = mean2(currentROI)
    end
    
    % VARIANCE
    if get(handles.checkbox_var,'Value') == 1
        var_value = var(double(currentROI(:)))
    end
    
    % SKEWNESS
    if get(handles.checkbox_skew,'Value') == 1
        skew_value = var(double(currentROI(:)))
    end
    
    % SKEWNESS
    if get(handles.checkbox_kurtosis,'Value') == 1
        kurtosis_value = var(double(currentROI(:)))
    end
    
    % CO-OCCURRENCE MATRIX
    glcm = graycomatrix(currentROI(:,:,1), 'GrayLimits',[]);
    [featureVector,coocMat] = cooc3d (currentROI);
