...
handles.defaultImageSeriesText = 'Enter name for ImageSeries';
...
if ImageAnalyzerStudy.imageSeriesId > 0
    ...
    warndlg('ImageSeries ID mismatch', 'ImageSeries ID error', 'modal')
    uiwait(gcf)
end
...
modaldlgTitle = 'Confirm ImageSeries deleting';
imageSeriesName = handles.study.imageSeriesList{ ...
    handles.study.imageSeriesId}.name;
modaldlgString = ['Delete ImageSeries "', imageSeriesName '"', ...
    ...
    case 'yes'
        ...
        if ~isempty(handles.study.imageSeriesList)
            ...
            if ishandle(hVisualization)
                ...
                if strcmp(currentImage{value}, ...
                        handles.study.imageSeriesList{ ...
                        handles.study.imageSeriesId}.name)
                    delete(getappdata(0, 'hVisualization'));
            ...
            % exist.
            fileStr = strcat(handles.study.pathName,'\',imageSeriesName, ...
                '\', imageSeriesName,'.is');
...
[fileName, pathName] = uigetfile( {'*.dcm', 'DICOM Files (*.dcm)'; ...
    '*.png', 'PNG Files (*.png)'; '*.bmp', 'BMP Files (*.bmp)'; ...
    '*.jpg', 'JPG Files (*.jpg)'}, ...
    'Load ImageSeries', 'MultiSelect', 'on');
